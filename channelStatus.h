...
void updateChannel(uint32_t *channelStatus, uint32_t *lastChannelStatus){
	...
	if (channelStatusUpdate && (millis() - channelStatusUpdate > 20)){
		uint32_t mask = (1UL << SHIFT_CH) - 1;
//
//		if (!*channelStatus && (*lastChannelStatus == ((1UL << SHIFT_CH) - 1))){
...
//			return ;
//		}
		if ( ((*channelStatus & mask) == mask) && ((*lastChannelStatus & 0xFF) == 0)){
			*channelStatus = 0;
			for (int8_t i = 7; i >= 0; i--){
				*channelStatus |= (1UL << i);
				*channelStatus |= (1UL << (i + 8));
				if (SHIFT_CH > 16){
					*channelStatus |= (1UL << (i + 16));
				}
				setChannelStatus(channelStatus, lastChannelStatus);
				delay(CHANGE_STATUS_DELAY);
			}
		...
		setChannelStatus(channelStatus, lastChannelStatus);setChannelStatus(channelStatus, lastChannelStatus);
	}
}
